# clip_finetune.py — imports + NLLModel + accurate_nb (tương thích PLC_Trainer)
...
from utils import euclidean_dist, kl_div  # giữ nguyên như file gốc
...
class NLLModel(nn.Module):
    def __init__(self, args):
        ...
        # Giữ đúng tên thuộc tính để PLC_Trainer dùng: self.models
        ...
        for _ in range(args.num_model):
            ...
            if feat_dim is None:
                feat_dim = getattr(getattr(backbone, "visual", None), "output_dim", None)
                if feat_dim is None:
                    raise RuntimeError("Cannot infer feature dim from OpenCLIP model.")
            self.models.append(backbone)
            self.dropouts.append(nn.Dropout(p=getattr(args, "plc_dropout", 0.1)))
            self.classifiers.append(nn.Linear(feat_dim, num_labels))

    # Giữ đúng chữ ký như BERT: input_ids, attention_mask, labels=None
    def forward(self, input_ids, attention_mask, labels=None):
        # Ở bản ảnh: input_ids == pixel_values; attention_mask bỏ qua
        x = input_ids.to(self.args.device, non_blocking=True)
        ...
        for i in range(len(self.models)):
            feats  = self.models[i].encode_image(x)                 # [B, D]
            logits = self.classifiers[i](self.dropouts[i](feats))   # [B, C]

            if labels is not None:
                loss_i = self.loss_fnt(logits, labels.to(self.args.device))
                losses.append(loss_i)
                probs.append(F.softmax(logits, dim=-1))

            # Giả lập hidden_states như BERT: tuple với last layer [B, 1, D],
            # để downstream dùng outputs[idx][-1][-1][:,0,:]
            hidden_states_fake = (feats.unsqueeze(1),)

            # BERT (labels=None) trả tuple bắt đầu bằng logits => outputs[idx][0] là logits
            outputs.append((logits, hidden_states_fake))
        ...
        if labels is not None:
            ...
            if num_models > 1:
                ...
                reg = sum(kl_div(avg_prob, p) for p in probs) / num_models
                loss = loss + self.args.alpha_t * reg.mean()
            return loss
